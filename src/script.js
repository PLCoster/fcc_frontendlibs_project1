...
function getQuote() {
  ...
  then(function (data) {
    ...
    let data = { quote: 'Do\'h! - The Simpson Quote API is offline!', character: 'Homer' };
    console.log(data);
    displayQuote(data);
  });
}
...
function displayQuote(data) {
  ...
  body.setAttribute('style', `background-image: url(${data[0].image}`);

  const tweetButton = document.querySelector('.tweet');
  //Truncate tweet to max 280 chars:
  if (quotedcont.length + 3 + data[0].character.length > 280) {
    quotedcont = quotedcont.slice(0, 280 - 6 - data[0].character.length) + '...';
  }
  tweetButton.setAttribute('href', `https://twitter.com/intent/tweet?related=freecodecamp&text=${quotedcont} - ${data[0].character}`);
}
